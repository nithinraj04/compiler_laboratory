        ...
        case NODE_WRITE: {
            ...
            if(root->left->nodetype == NODE_ID) {
                if(root->left->gstEntry == NULL) {
                    printf("Error: Undeclared variable '%s' in %s statement\n", root->left->varname, root->nodetype == NODE_READ ? "read" : "write");
                    exit(1);
                }
                if(root->left->gstEntry->size > 1) {
                    printf("Error: Cannot read into un-indexed array '%s' in %s statement\n", root->left->varname, root->nodetype == NODE_READ ? "read" : "write");
                    exit(1);
                }
                if(root->left->gstEntry->ptr_level > 0) {
                    printf("Error: Cannot read into pointer variable '%s' without dereferencing in %s statement\n", root->left->varname, root->nodetype == NODE_READ ? "read" : "write");
                    exit(1);
                }
            }
            if(root->left->nodetype == NODE_ARRAY) {
                if(root->left->gstEntry == NULL) {
                    printf("Error: Undeclared array '%s' in %s statement\n", root->left->varname, root->nodetype == NODE_READ ? "read" : "write");
                    exit(1);
                }
                if (root->right && root->right->nodetype == NODE_NUM) {
                    int index = root->right->val;
                    if (index < 0 || index >= root->left->gstEntry->size) {
                        printf("Error: Array index out of bounds for array '%s' in %s statement (size is %d, index accessed is %d)\n", root->left->varname, root->nodetype == NODE_READ ? "read" : "write", root->left->gstEntry->size, index);
                        exit(1);
                    }
                }
            }
            if(root->left->nodetype == NODE_PTR) {
                if(root->left->gstEntry == NULL) {
                    printf("Error: Undeclared pointer variable '%s' in %s statement\n", root->left->right->varname, root->nodetype == NODE_READ ? "read" : "write");
                    exit(1);
                }
                if(getDerefLevel(root->left) != 0) {
                    printf("Error: Pointer variable '%s' incorrectly dereferenced in %s statement\n", root->left->right->varname, root->nodetype == NODE_READ ? "read" : "write");
                    exit(1);
                }
            }
            return;
